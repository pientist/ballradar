...
class Postprocessor:
    def __init__(self, traces: pd.DataFrame, pred_poss: pd.DataFrame, pred_traces: pd.DataFrame):
        ...
        self.output = pd.DataFrame(index=self.traces.index, columns=["carrier", "ball_x", "ball_y"])
    ...
    def finetune_ball_trace(
        ...
        output = pd.DataFrame(index=player_traces.index, columns=["carrier", "ball_x", "ball_y"])
        ...
        output[["ball_x", "ball_y"]] = output[["ball_x", "ball_y"]].astype(float).interpolate(limit_direction="both")
        return output
        ...
        for phase in self.traces["phase"].unique():
            ...
            if method == "ball_accel":
                ...
                for episode in tqdm(episodes, desc=f"Phase {phase}"):
                    ...
                    carry_records = Postprocessor.detect_carries_by_accel(
                        ep_traces, ep_pred_traces, players, thres_accel
                    )
